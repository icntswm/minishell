...
/*   Updated: 2021/11/09 18:29:14 by squickfi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_files	*fill_list(t_pipes *pipes, char redrct)
{
	...
	{
		if (q->next == NULL)
			p = list_file(p, ft_substr(pipes->cmd, ++q->pos, end_file(\
			pipes->cmd, q->pos, ft_strlen(pipes->cmd) - q->pos)), q->red);
		else
			p = list_file(p, ft_substr(pipes->cmd, ++q->pos, end_file(\
			pipes->cmd, q->pos, q->next->pos - q->pos)), q->red);
		q->pos--;
		q = q->next;
	}
	return (p);
