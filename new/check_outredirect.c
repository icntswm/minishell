...
void	check_outredirect(t_data *data)
{
	...
	while (p != NULL)
	{
		...
		if (p->cmd[i] == 34)
			...
			ft_error(&(*data), "pipe");
		else if (p->cmd[i] == '>' && p->cmd[i + 1] == '>' && p->cmd[i + 2] == '>' && p->cmd[i + 3] != '>' && p->cmd[i + 3] != '|')
			ft_error(&(*data), "one_outred");
			...
			ft_error(&(*data), "one_inred");
		else if (p->cmd[i] == '>' && p->cmd[i + 1] == '>' && p->cmd[i + 2] == '<' && p->cmd[i + 3] == '<' && p->cmd[i + 4] != '<')
			ft_error(&(*data), "two_inred");
		else if (p->cmd[i] == '>' && p->cmd[i + 1] == '>' && p->cmd[i + 2] == '<' && p->cmd[i + 3] == '<' && p->cmd[i + 4] == '<')
			ft_error(&(*data), "three_inred");
		else if (p->cmd[i] == '>' && p->cmd[i + 1] == ' ')
