...
void    parser(t_data *data)
{
	...
	else
	{
		...
		else
		{
			...
			// {
			// 	// printf("error: %s %d %d\n", data->error, ft_strncmp(data->error, "warning: here-document delimited", ft_strlen("warning: here-document delimited")),
			// 	// 	ft_strncmp(data->error, "ctrl_c_redirect_heredoc", ft_strlen("ctrl_c_redirect_heredoc")));
			// 	// if (data->error[0] != '\0')
			// 		printf("minishell11: %s\n", data->error);
			// }
		}
	}
	// printf("line: %s\n", data->line);
